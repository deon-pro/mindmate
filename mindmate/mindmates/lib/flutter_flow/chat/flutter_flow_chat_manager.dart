...
class FFChatInfo {
  ...
  //           if (chatRecord.users.length > 3) {
  //             int number = chatRecord.users.length - 3;
  //             final display =
  //                 '${m.displayName}  + $number other]${number > 1 ? 's' : ''}';
  //             return display;
  //           }
  ...
  String chatPreviewTitle() {
    ...
    // final numOthers = chatRecord.users.length - otherUsersList.length;
    final numOthers = chatRecord.users.length - 3;


    // Get the names of the first 3 users (or all if there are fewer than 3)
    final names = otherUsersList.take(3).map((m) {
    ...
    // Add '+ x more' if there are more than 3 users
    if (chatRecord.users.length > 3) {
      names.add('+$numOthers more${numOthers > 1 ? '' : ''}');
    }
...
class FFChatManager {
    ...
    return chatStream.asyncMap((chat) async {
      ...
      if (chatRecord != null) {
        ...
        userRefs = {
          ...
          ...(userRefs.toSet()..removeAll(userAndSender)),
        };
      }
  ...
  Future<ChatsRecord?> createChat(List<DocumentReference> otherUsers, {required String groupName}) async {
